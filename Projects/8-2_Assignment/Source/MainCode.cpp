...
// MoveOneStep method)
const char* const SW_VERSION = "20260105a"; // do not delete this line
...
public:
	...
	void MoveOneStep()
	{
		...
		{
			if (y < 1 - radius) // i.e., y has not touched the top of the screen
			{
				y += speed; // y = y + speed; (aka, move y "speed" amount in toward the top)
			}
			else
			{
				// y has hit the top of the screen, therefore, assign a new direction
				direction = GetRandomDirection();
			}
		}
...
// the world vector contains circles
vector<Circle> world;

...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// this callback is used to receive mouse scroll events (speed control)
	glfwSetScrollCallback(window, &ViewManager::Mouse_Scroll_Callback);

	// tell GLFW to capture all mouse events
	glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
	// keep key presses available until they are polled
...
void ViewManager::Mouse_Position_Callback(GLFWwindow* window, double xMousePos, double yMousePos)
{
	...
	// apply mouse delta to camera yaw/pitch
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// W/S = zoom, A/D = pan, Q/E = vertical move
	...
	// process camera movement up and down (world-space)
	if (glfwGetKey(m_pWindow, GLFW_KEY_Q) == GLFW_PRESS)
	{
		// move up while Q is held
		g_pCamera->Position += g_pCamera->WorldUp * g_pCamera->MovementSpeed * gDeltaTime;
		// std::cout << "Q: move up" << std::endl; // debug: confirm Q input is detected
	}
	if (glfwGetKey(m_pWindow, GLFW_KEY_E) == GLFW_PRESS)
	{
		// move down while E is held
		g_pCamera->Position -= g_pCamera->WorldUp * g_pCamera->MovementSpeed * gDeltaTime;
		// std::cout << "E: move down" << std::endl; // debug: confirm E input is detected
	}
}

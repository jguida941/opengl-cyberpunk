...
private:
	...
	// optional second texture for detail blending on top of base texture
	void SetShaderDetailTexture(
		std::string textureTag,
		float blendFactor,
		float uScale,
		float vScale);
	...
	void DrawPicnicTable(const glm::vec3& centerPosition, float yRotationDegrees);
	void DrawSodaCan(const glm::vec3& baseCenterPosition, float radius, float height);
	void DrawPizzaSlice(const glm::vec3& centerPosition, float yRotationDegrees);
	void DrawDice(const glm::vec3& centerPosition, float edgeLength);
	void DrawBall(const glm::vec3& centerPosition, float radius);
